...
async function fetchRecentHot(params = {}) {
    ...
    const dynamicKey = `recent-${category}-${page}`;
    if (globalData.dynamic[dynamicKey]) {
        console.log(`[BGM Widget v10.4] 命中近期热门动态缓存: p${page}`);
        return globalData.dynamic[dynamicKey];
    }

    console.log(`[BGM Widget v10.4] 近期热门未命中预构建数据，启动动态获取: p${page}`);
